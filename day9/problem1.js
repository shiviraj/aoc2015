...
const findNearestCity = (distances, location, visitedCities) => {
  return distances.filter(({locations}) => locations.includes(location) && !isVisitedBothCities(visitedCities, locations))
    .reduce((nearestCity, location) => (nearestCity.distance > location.distance) ? location : nearestCity)
}
...
const findDistances = (allLocations, distances) => {
  ...
  allLocations.forEach(location => {
    ...
    while (allLocations.length !== visitedCities.length) {
      const {locations, distance} = findNearestCity(distances, location, visitedCities)
